...
**Task** In the console (look below), type 2+2 and hit enter. What does it return? Type your answer below. **Answer:** [1] 4 \### Task 2: Variables
...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer** <chr> <dbl>
...
In the space below, write the name of the business from the first row of the "title" column. **Answer** Directed Research %>% %>% 
...
**Answer** I think it is grouping the classes by term, summarizing them, and counting them.
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** It is now arranging it from largest count_classes to smallest
...
**Answer** It is now arranged by department
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** It added the total seats 
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer** Added mean and median seats. Term, count_classes, and total_seats are the same.
...
1.  Why are there so few women's studies classes compared to other subjects?

2.  Why are there 4 different agriculture departments?

3.  Why is mean class size for many stem subjects larger than those for humanities?
