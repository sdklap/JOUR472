...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer** They do not have the same column structure. 2016 and 2018 have the same structure, but nothing else. They all have the same colum titles though, except total is lowercase in 2022 and uppercase in the rest.
...
This has total and TOTAL as different columns. After fixing this, this combines all 4 datasets into one.
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer** There are increasing numbers of voters and there are more democrats registering than republicans. There has also been a big jump in unafilliated as well as other.
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24 rows and there should be 24 rows.
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** This contains how many people in each county are what race.
...
view(maryland_population_with_voters_and_demographics)
```

### Task 11: Asking Demographic Questions

**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** PG county has the highest black population and highest percent of democrats.
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** There are much fewer Asian people in Maryland. It still checks out though that the places with minorities vote more democratic.
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer** It is a dbl
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer** Now, it is chr
