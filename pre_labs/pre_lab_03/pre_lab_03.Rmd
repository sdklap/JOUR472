...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** There are no parsing errors.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** Vendor name is chr or character, amount is dbl or number, date is chr or character
...
**Answer** 549 zip codes do not have 5 characters. I think that these could be the plus 4 codes at the end of a zip code. 
...
**Answer** The first line, md_grants_loans |> , is telling r what dataset to work with. group_by(`Fiscal Year`, Category) |> is saying to show the fiscal year and the categories of the fiscal year, grant, loan, and contract.   summarize(count = n()) |> is counting the number of grants and loans.  arrange(`Fiscal Year`) is sorting the chart by fiscal year starting in 2009. The thing that seems problematic to me is that there are many more grants than loans.
...
**Answer** I see the smallest grant is for $60 in 2009 to Land Preservation Trust, Inc. from Department of Natural Resources/Rural Legacy Program. All of the grants on the first page are under $500
...
**Answer**  all of the headers are lowercase and if there are two words there is an underscore between them
...
**Answer** cleaned_md_grants_loans <- md_grants_loans |> is showing what dataset we're working with.  clean_names() |> is saying we need to clean the names. rename(source = grantor) |> 
  mutate(source = str_to_upper(source), grantee = str_to_upper(grantee), description = and  str_to_upper(description)) are changing how the names are viewed, grantor is now source and the description, grantee, and source are now all in uppercase.
...
**Answer** There are 17 names for 50 duplicates.
...
**Answer** There are now 17,740 rows.
...
**Answer** This column only shows the first 5 digits of a zip code.
...
**Answer** 21202 has gotten the most amount of money from the Maryland Tourism Board. This makes sense, because it is the zip code to Baltimore, which is Maryland's most populated city. The top result is a grant to STAR-SPANGLED 200 INC from DEPARTMENT OF BUSINESS AND ECONOMIC DEVELOPMENT - MARYLAND TOURISM BOARD	for $2,172,512.
