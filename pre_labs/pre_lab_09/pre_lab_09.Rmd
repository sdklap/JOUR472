...
https://www.datawrapper.de/_/crkWw/

```{r, echo=FALSE}
knitr::include_graphics("images/crkWw-cox-flops.png")
```
...
**Task** Write code to add columns showing the total number of votes for each county and the percentage of votes received by Wes Moore in each county (you did this in Lab), then replace the CSV file in the data folder with it.
...
md_gov_county <- md_gov_county |> 
  ...
  mutate(moore_pct = Moore/total_votes)
...
##There are no changes to my map. 
...
Now, when you hover over a county, the name not the fips code shows up as well as the percent of voters. 

##This changed the FIPS code to the name of the county and the decimal to a percent.
...
https://www.datawrapper.de/_/2i8G8/
